...
// KIS OAuth 토큰 발급 프록시
exports.kisToken = (0, https_1.onRequest)({ region: "asia-northeast3", cors: ALLOWED_ORIGIN, invoker: "public" }, async (req, res) => {
    if (req.method !== "POST") {
...
exports.kisApprovalKey = (0, https_1.onRequest)({ region: "asia-northeast3", cors: ALLOWED_ORIGIN, invoker: "public" }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).send("Method Not Allowed");
        return;
    }
...
exports.kisBalance = (0, https_1.onRequest)({ region: "asia-northeast3", cors: ALLOWED_ORIGIN, invoker: "public" }, async (req, res) => {
    if (req.method !== "GET") {
        res.status(405).send("Method Not Allowed");
        return;
    }
    const { token, appkey, appsecret, accountNo, fk100 = "", nk100 = "" } = req.query;
    ...
    try {
        ...
        const response = await fetch(`${KIS_BASE}/uapi/domestic-stock/v1/trading/inquire-balance?${params}`, { headers: { Authorization: `Bearer ${token}`, appkey, appsecret, tr_id: "TTTC8434R" } });
        const data = await response.json();
        res.status(response.status).json(data);
    }
    catch (e) {
...
exports.kisOrder = (0, https_1.onRequest)({ region: "asia-northeast3", cors: ALLOWED_ORIGIN, invoker: "public" }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).send("Method Not Allowed");
        return;
    }
    const { token, appkey, appsecret, cano, acntPrdtCd, side, PDNO, ORD_DVSN, ORD_QTY, ORD_UNPR } = req.body;
    ...
    try {
        const response = await fetch(`${KIS_BASE}/uapi/domestic-stock/v1/trading/order-cash`, {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
                CANO: cano, ACNT_PRDT_CD: acntPrdtCd,
                PDNO, ORD_DVSN, ORD_QTY, ORD_UNPR: ORD_UNPR !== null && ORD_UNPR !== void 0 ? ORD_UNPR : "0",
            }),
        });
        const data = await response.json();
        res.status(response.status).json(data);
...
exports.kisInquirePsbl = (0, https_1.onRequest)({ region: "asia-northeast3", cors: ALLOWED_ORIGIN, invoker: "public" }, async (req, res) => {
    if (req.method !== "GET") {
        res.status(405).send("Method Not Allowed");
        return;
    }
    const { token, appkey, appsecret, cano, acntPrdtCd, PDNO, ORD_UNPR, ORD_DVSN } = req.query;
    ...
    try {
        const params = new URLSearchParams({
            CANO: cano, ACNT_PRDT_CD: acntPrdtCd,
            PDNO, ORD_UNPR, ORD_DVSN: ORD_DVSN !== null && ORD_DVSN !== void 0 ? ORD_DVSN : "00",
            CMA_EVLU_AMT_ICLD_YN: "N", OVRS_ICLD_YN: "N",
        });
        const response = await fetch(`${KIS_BASE}/uapi/domestic-stock/v1/trading/inquire-psbl-order?${params}`, {
            headers: {
                Authorization: `Bearer ${token}`,
...
exports.kisDailyOrder = (0, https_1.onRequest)({ region: "asia-northeast3", cors: ALLOWED_ORIGIN, invoker: "public" }, async (req, res) => {
    if (req.method !== "GET") {
        res.status(405).send("Method Not Allowed");
        return;
    }
    const { token, appkey, appsecret, accountNo, ccldDvsn = "00", fk100 = "", nk100 = "" } = req.query;
    if (!token || !appkey || !appsecret || !accountNo) {
        res.status(400).json({ error: "required params missing" });
        return;
    }
    ...
    try {
        ...
        const response = await fetch(`${KIS_BASE}/uapi/domestic-stock/v1/trading/inquire-daily-ccld?${params}`, { headers: { Authorization: `Bearer ${token}`, appkey, appsecret, tr_id: "TTTC8001R", custtype: "P" } });
        const data = await response.json();
        res.status(response.status).json(data);
    }
    catch (e) {
